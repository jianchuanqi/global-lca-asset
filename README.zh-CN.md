...
当前版本为 `2026-09-01.2`，证据截止日期为 2026 年 9 月 1 日。
...
| 核心数据库家族 | 80 |
| 扩展数据承载资产 | 88 |
| PCF/LCA 软件产品、API、模型与工作流 | 130 |
| 符合本轮 PCF/LCA 市场纳入规则的记录 | 97 |
| 已审查的软件候选目录条目 | 104 |
| 有公开证据的软件主体—角色断言 | 179 |
| 保留待核实的软件角色标签 | 39 |
| 公开证据记录 | 339 |
...
所有数量都是在已发布纳入规则和证据截止日期下可复现的下限，并不表示已经得到最终的全球总数。软件市场审查将产品形态、对齐后的核心功能、多值能力标签以及标准/网络关联分开。“PACT”特指 WBCSD Partnership for Carbon Transparency，仅作为互操作证据保留，不作为软件类别或质量评级。
...
`data/seed/inventory-v2.public.json` 与有记录的 curated 审查层共同构成经过隐私筛选的权威公开输入。确定性构建流程在 `data/package/current/` 生成版本化数据包，包括：
...
- 数据集数量和资产类别覆盖统计；
- 数据库范围、获取方式、格式、PCF/LCA 软件与公司角色、开发者与行业、mapping 等主题视图；
