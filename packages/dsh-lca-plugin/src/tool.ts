...
interface ToolOptions {
  ...
  output: {
    schema: AuthorSchema
    render(args: ToolArgs, value: any): Array<{ type: 'text'; text: string }>
...
function compileValue(spec: AuthorSchema): Record<string, unknown> {
  ...
  if (spec.type === 'array' && spec.items !== undefined) result.items = compileValue(spec.items)
  return result
}

function compileParameters(parameters: Record<string, AuthorSchema>): Record<string, unknown> {
  const properties: Record<string, unknown> = {}
  const required: string[] = []
...
function validateValue(spec: AuthorSchema, value: unknown, path: string): string[] {
  if (spec.type === 'json') return []
  if (spec.type === 'null') return value === null ? [] : [`${path} must be null`]
  if (spec.type === 'string') return typeof value === 'string' ? [] : [`${path} must be a string`]
  if (spec.type === 'boolean') return typeof value === 'boolean' ? [] : [`${path} must be a boolean`]
  if (spec.type === 'number') {
  ...
  for (const [key, child] of Object.entries(spec.properties ?? {})) {
    if (!(key in object)) {
      if (child.required === true) violations.push(`${path}.${key} is required`)
      continue
    }
  ...
  if (spec.additionalProperties === false) {
    ...
    for (const key of Object.keys(object)) {
      if (!allowed.has(key)) violations.push(`${path}.${key} is not allowed`)
    }
  }
  return violations
}
...
export function defineTool(options: ToolOptions): unknown {
  const parameterSpec: AuthorSchema = {
    type: 'object',
  ...
  return {
    ...
    async execute(args: unknown, exec: ToolExecutionContext): Promise<JsonValue> {
      ...
      return await options.execute(args as ToolArgs, exec) as JsonValue
    },
    ...(options.presentCall === undefined ? {} : {
      presentCall(args: unknown): ToolView | undefined {
        if (validateValue(parameterSpec, args, 'arguments').length > 0) return undefined
        try {
          return options.presentCall!(args as ToolArgs)
        } catch {
          return undefined
        }
      },
    ...
    ...(options.presentResult === undefined ? {} : {
      presentResult(args: unknown, result: unknown): ToolView | undefined {
        if (validateValue(parameterSpec, args, 'arguments').length > 0) return undefined
        try {
          return options.presentResult!(args as ToolArgs, result)
        } catch {
          return undefined
        }
      },
    ...
    ...(options.isConcurrencySafe === undefined ? {} : {
      isConcurrencySafe(args: unknown): boolean {
        if (validateValue(parameterSpec, args, 'arguments').length > 0) return false
        try {
          return options.isConcurrencySafe!(args as ToolArgs) === true
        } catch {
          return false
        }
      },
